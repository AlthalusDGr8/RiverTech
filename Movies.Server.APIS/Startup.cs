...
namespace Movies.Server.APIS
{
	public class Startup
    {
		...
		private string dbName = $"Test_Orleans";

		private void PrepareDatabase()
		{
			

			ExecuteScripts(new string[] { $"CREATE DATABASE {dbName}" }, ConnectionString);

			var connectionString = $@"{ConnectionString}Initial Catalog={dbName}";
			
			ExecuteScripts(new string[] { @"DECLARE @current NVARCHAR(256);
...
(
    ...
    WHERE
		...
		private void OnShutdown()
		{
			...
			var scripts = new string[] { @$"ALTER DATABASE {dbName} 
                                            SET SINGLE_USER 
